...
function navigateTo(href, hash) {
    // Не пушим состояние если URL не меняется (предотвращает двойной клик «Назад»)
    const isSameUrl = href === location.href || href === location.origin + location.pathname;
    const main = document.querySelector('main');
    if (main) main.classList.add('is-transitioning');
    setTimeout(async () => {
        if (!isSameUrl) history.pushState({ url: href }, '', href);
        tocInit = false;
...
async function initApp() {
    ...
    // Помечаем начальную запись истории — чтобы popstate всегда получал корректный state
