...
// --- URL NORMALIZATION ---
// Fixes duplicate slashes and uppercase letters, preserving the 'Tika' base segment.
// Uses replaceState — no reload, no history entry added.
function normalizeUrl() {
    ...
    // 2. Split into non-empty segments — collapses ALL duplicate slashes automatically
    const segments = path.split('/').filter(s => s.length > 0);

    // 3. Fix case: first segment must be exactly 'Tika' (GitHub Pages is case-sensitive),
    //    all other segments → lowercase
    if (segments.length > 0 && segments[0].toLowerCase() === 'tika') {
        segments[0] = 'Tika';
    }
    ...
    // 4. Rebuild with leading slash and trailing slash
    const clean = '/' + segments.join('/') + '/';
    const normalized = clean + search + hash;
    const current = raw + search + hash;
...
// --- CUSTOM BACKGROUND ---
let _bgHtmlCache = null;

function applyCustomBackground(html) {
    const hasHtml = html && html.trim();

    // Skip if same content already displayed
    if (hasHtml && html === _bgHtmlCache) return;
    _bgHtmlCache = hasHtml ? html : null;
    ...
    if (!hasHtml) {
        ...
        if (iframe) {
            ...
            iframe.onload = null;
            setTimeout(() => { if (iframe) iframe.src = 'about:blank'; }, 600);
        }
        return;
    }
    ...
    // Revoke old blob URL
    if (iframe._blobUrl) {
        const old = iframe._blobUrl;
        iframe._blobUrl = null;
        setTimeout(() => URL.revokeObjectURL(old), 1000);
    }

    // Always set onload BEFORE src
    iframe.onload = null; // clear previous handler first
    const newBlob = new Blob([html], { type: 'text/html' });
    const blobUrl = URL.createObjectURL(newBlob);
    iframe._blobUrl = blobUrl;

    iframe.addEventListener('load', function onLoaded() {
    ...
    // Fallback: if load event doesn't fire (e.g. same-origin blob cached), force visibility
    setTimeout(() => {
        if (iframe && iframe._blobUrl === blobUrl) {
...
function hydrateHome(lang) {
    ...
    // backgroundHtml: prefer Firebase data, fall back to statically embedded site-config
    const bgHtml = siteData.home?.backgroundHtml || siteConfig.homeData?.backgroundHtml || '';
...
function restoreHomeDom() {
    const mainEl = document.querySelector('main');
    if (!mainEl) return;
    // Reset background cache so applyCustomBackground re-applies on next hydrateHome
    _bgHtmlCache = null;
    mainEl.innerHTML = `
    ...
    // Remove is-visible from background iframe so it fades back in after re-apply
    const bgIframe = document.getElementById('custom-background-iframe');
    if (bgIframe) bgIframe.classList.remove('is-visible');
    tocInit = false;
}
...
async function initApp() {
    normalizeUrl(); // fix slashes and case before anything else
