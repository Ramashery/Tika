...
// Вычисляет относительную яркость цвета (0 = чёрный, 1 = белый) по стандарту WCAG
...
// Слушатель ответов от iframe — получаем средний цвет фона
window.addEventListener('message', e => {
    if (e.data && e.data.type === 'tika-bg-color') {
        const { r, g, b } = e.data;
        applyBgClass(relativeLuminance(r, g, b) > 0.35);
    }
});

// Зонд, который инжектируется в HTML кастомного фона.
// Сначала пробует прочитать пиксели canvas (для WebGL/p5.js/Three.js фонов),
// затем fallback на background-color элементов.
const BG_PROBE_SCRIPT = `
<script>
(function() {
    function avg(arr) {
        var sum = [0,0,0], n = arr.length / 4;
        for (var i = 0; i < arr.length; i += 4) { sum[0]+=arr[i]; sum[1]+=arr[i+1]; sum[2]+=arr[i+2]; }
        return n > 0 ? [sum[0]/n, sum[1]/n, sum[2]/n] : null;
    }
    function parseColor(str) {
        if (!str) return null;
        var m = str.match(/\\d+/g);
        if (m && m.length >= 3 && +m[3] !== 0) return [+m[0], +m[1], +m[2]];
    ...
    function probe() {
        // 1. Пробуем прочитать пиксели из canvas (WebGL / p5.js / Three.js)
        var canvases = document.querySelectorAll('canvas');
        for (var i = 0; i < canvases.length; i++) {
            try {
                var cv = canvases[i];
                if (cv.width === 0 || cv.height === 0) continue;
                // Для WebGL нужен preserveDrawingBuffer=true; пробуем напрямую
                var ctx2d = cv.getContext('2d');
                if (ctx2d) {
                    var px = ctx2d.getImageData(0, 0, cv.width, cv.height).data;
                    var c = avg(px);
                    if (c && (c[0] + c[1] + c[2]) > 0) { send(c[0], c[1], c[2]); return; }
                }
                // Пробуем скопировать WebGL canvas в 2d canvas
                var tmp = document.createElement('canvas');
                tmp.width = Math.min(cv.width, 64); tmp.height = Math.min(cv.height, 64);
                var tc = tmp.getContext('2d');
                tc.drawImage(cv, 0, 0, tmp.width, tmp.height);
                var px2 = tc.getImageData(0, 0, tmp.width, tmp.height).data;
                var c2 = avg(px2);
                if (c2 && (c2[0] + c2[1] + c2[2]) > 0) { send(c2[0], c2[1], c2[2]); return; }
            } catch(e) {}
        }
        // 2. Fallback: читаем computed background-color body, html и первых детей body
        var els = [document.body, document.documentElement].concat(
            Array.prototype.slice.call(document.body.children, 0, 5)
        );
        for (var j = 0; j < els.length; j++) {
            try {
                var bg = getComputedStyle(els[j]).backgroundColor;
                var c3 = parseColor(bg);
                // пропускаем прозрачное/чёрное
                if (c3 && (c3[0] + c3[1] + c3[2]) > 10) { send(c3[0], c3[1], c3[2]); return; }
            } catch(e) {}
        }
        // 3. Ничего не нашли — считаем тёмным
        send(0, 0, 0);
    }
    // Пробуем сразу, потом ещё раз через 800ms (после того как canvas успеет отрисоваться)
...
function applyCustomBackground(html) {
    ...
    const instrumented = html.includes('</body>')
        ? html.replace('</body>', BG_PROBE_SCRIPT + '</body>')
        : html + BG_PROBE_SCRIPT;
...
function handleClick(e) {
    ...
    // Same path + anchor → just scroll (replaceState, не pushState — чтобы не плодить лишние записи в истории)
    if (url.pathname === location.pathname && url.hash) {
        setTimeout(() => { history.replaceState({}, '', url.href); scrollToHash(url.hash); }, delay);
        return;
    }
