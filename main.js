...
const EYEBROW = { ru: 'TINATIN CHALATASHVILI', ka: 'თინათინ ჩალათაშვილი' };
...
function applyBgClass(isLight) {
    document.body.classList.toggle('light-bg', isLight);
}

window.addEventListener('message', e => {
    if (!e.data || typeof e.data !== 'object') return;
    if (e.data.type === 'tika-theme') {
        applyBgClass(e.data.theme === 'light');
    }
});

// Зонд инжектируется в HTML кастомного фона.
// Для CSS/статичных фонов: читаем цвет из html, body, :root и первых дочерних элементов.
// Поддерживает явный флаг: <meta name="tika-theme" content="light|dark">
const BG_PROBE = `<script>
(function(){
  ...
  function parseRgb(s){
    ...
    // пропускаем если альфа=0 (прозрачный)
    if(m.length>=4&&parseFloat(m[3])===0)return null;
    return[+m[0],+m[1],+m[2]];
  }
  function send(light){
    window.parent.postMessage({type:'tika-theme',theme:light?'light':'dark'},'*');
  }
  function probe(){
    // 1. Явный мета-тег — наивысший приоритет
    var meta=document.querySelector('meta[name="tika-theme"]');
    if(meta){send(meta.getAttribute('content')==='light');return;}
    // 2. Явный атрибут
    var tagged=document.querySelector('[data-tika-theme]');
    if(tagged){send(tagged.getAttribute('data-tika-theme')==='light');return;}
    // 3. Берём пиксель из центра экрана через elementFromPoint
    //    (работает для CSS backgrounds, градиентов, любых видимых элементов)
    try{
      var el=document.elementFromPoint(window.innerWidth/2,window.innerHeight/2)||document.body;
      // Поднимаемся по дереву, ищем непрозрачный background-color
      var cur=el;
      while(cur&&cur!==document.documentElement.parentElement){
        var bg=getComputedStyle(cur).backgroundColor;
        var rgb=parseRgb(bg);
        if(rgb&&(rgb[0]+rgb[1]+rgb[2])>0){
          send(luminance(rgb[0],rgb[1],rgb[2])>.35);
          return;
        }
        cur=cur.parentElement;
      }
    }catch(e){}
    // 4. Перебираем html, body и первые 8 дочерних элементов body
    var candidates=[document.documentElement,document.body]
      .concat(Array.prototype.slice.call(document.body.children,0,8));
    for(var i=0;i<candidates.length;i++){
      try{
        var bg=getComputedStyle(candidates[i]).backgroundColor;
        var rgb=parseRgb(bg);
        if(rgb&&(rgb[0]+rgb[1]+rgb[2])>3){
          send(luminance(rgb[0],rgb[1],rgb[2])>.35);
          return;
        }
      }catch(e){}
    }
    // 5. Читаем canvas если есть
    ...
    if(cv&&cv.width&&cv.height){
      try{
        ...
        tc.drawImage(cv,Math.floor(cv.width/2-4),Math.floor(cv.height/2-4),8,8,0,0,8,8);
        var px=tc.getImageData(0,0,8,8).data;
        var r=0,g=0,b=0;
        for(var j=0;j<px.length;j+=4){r+=px[j];g+=px[j+1];b+=px[j+2];}
        var n=px.length/4;
        if(r+g+b>0){send(luminance(r/n,g/n,b/n)>.35);return;}
      }catch(e){}
    }
    // 6. Ничего не нашли — тёмный
    send(false);
  }
  // Запускаем после загрузки DOM и ещё раз через 500ms для динамических фонов
  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',function(){probe();setTimeout(probe,500);});
  } else {
    probe();setTimeout(probe,500);
  }
})();
<\/script>`;
...
function applyCustomBackground(html) {
    ...
    const instrumented = html.includes('</body>')
        ? html.replace('</body>', BG_PROBE + '</body>')
        : html + BG_PROBE;
...
function initToc(lang) {
    if (_tocAbort) { _tocAbort.abort(); _tocAbort = null; }
    ...
    const sig = { signal: _tocAbort.signal };
    ...
    document.addEventListener('click', e => {
        if (panel.classList.contains('is-visible') && !wrapper.contains(e.target)) close();
    }, sig);

    panel.addEventListener('click', e => {
        if (e.target.closest('a')) setTimeout(close, 100);
    }, sig);
...
// --- NAVIGATION ---
// Токен для отмены незавершённого navigateTo при нажатии "назад"
let _navToken = 0;

// Single helper: fade → pushState → render → fade in → scroll
function navigateTo(href, hash) {
    const main = document.querySelector('main');
    const token = ++_navToken;          // уникальный токен этого перехода
    if (main) main.classList.add('is-transitioning');
    setTimeout(async () => {
        if (token !== _navToken) {      // навигация была отменена (нажали "назад")
            if (main) main.classList.remove('is-transitioning');
            return;
        }
        history.pushState({ navToken: token }, '', href);
        tocInit = false;
    ...
    }, 300);                            // уменьшили с 400 до 300ms — меньше окно для race condition
}
...
function handleClick(e) {
    ...
    // Same path + anchor → просто скроллим, НЕ добавляем запись в историю
    if (url.pathname === location.pathname && url.hash) {
        setTimeout(() => scrollToHash(url.hash), delay);
        return;
    }
...
async function initApp() {
    ...
    // popstate: пользователь нажал "Назад"/"Вперёд"
    window.addEventListener('popstate', () => {
        ++_navToken;                    // отменяем любой незавершённый navigateTo
        const main = document.querySelector('main');
        if (main) main.classList.remove('is-transitioning');   // снимаем fade-out если был
        tocInit = false;
        routeAndRender();
    });

    // pageshow: обработка bfcache restore (браузер достаёт страницу из кэша)
    window.addEventListener('pageshow', e => {
        if (e.persisted) {              // страница восстановлена из bfcache
            ++_navToken;
            routeAndRender();
        }
    });
