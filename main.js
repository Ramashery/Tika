...
// Слушатель postMessage от iframe
// Поддерживаем два варианта:
//   1. Явный: { type: 'tika-theme', theme: 'light' | 'dark' }
//   2. Цветовой: { type: 'tika-bg-color', r, g, b }
window.addEventListener('message', e => {
    if (!e.data || typeof e.data !== 'object') return;
    if (e.data.type === 'tika-theme') {
        applyBgClass(e.data.theme === 'light');
    } else if (e.data.type === 'tika-bg-color') {
        const { r, g, b } = e.data;
        ...
        applyBgClass(lum > 0.35);
    }
});

// Зонд, который инжектируется в HTML кастомного фона.
// Определяет тему тремя способами (по убыванию надёжности):
//   1. Явный мета-тег: <meta name="tika-theme" content="light">
//   2. Явный атрибут: data-tika-theme="light" на любом элементе
//   3. Чтение пикселей canvas (2D) и background-color body/html
const BG_PROBE = `<script>
(function(){
  function send(isLight){
    window.parent.postMessage({type:'tika-theme',theme:isLight?'light':'dark'},'*');
  }
  function probe(){
    // 1. Явный мета-тег
    var meta = document.querySelector('meta[name="tika-theme"]');
    if(meta){ send(meta.content==='light'); return; }
    // 2. Явный data-атрибут
    var el = document.querySelector('[data-tika-theme]');
    if(el){ send(el.dataset.tikaTheme==='light'); return; }
    // 3. Читаем пиксели первого найденного canvas
    ...
    for(var i=0;i<canvases.length;i++){
      try{
        ...
        tc.drawImage(cv,0,0,32,32);
        var d=tc.getImageData(0,0,32,32).data;
        var r=0,g=0,b=0,n=d.length/4;
        for(var j=0;j<d.length;j+=4){r+=d[j];g+=d[j+1];b+=d[j+2];}
        r/=n;g/=n;b/=n;
        if(r+g+b>10){
          window.parent.postMessage({type:'tika-bg-color',r:r,g:g,b:b},'*');
          return;
        }
      }catch(e){}
    }
    // 4. background-color body/html
    var els=[document.body,document.documentElement];
    for(var k=0;k<els.length;k++){
      var bg=getComputedStyle(els[k]).backgroundColor;
      var m=bg.match(/\\d+(\\.\\d+)?/g);
      if(m&&m.length>=3&&+m[0]+\+m[1]+\+m[2]>15){
        window.parent.postMessage({type:'tika-bg-color',r:+m[0],g:+m[1],b:+m[2]},'*');
        return;
      }
    }
    // 5. Ничего не нашли — считаем тёмным
    send(false);
  }
  // Запускаем несколько раз: сразу, через 300ms и через 1200ms
  // чтобы поймать момент после первого кадра анимации
  if(document.readyState==='complete'){probe();setTimeout(probe,300);setTimeout(probe,1200);}
  else{window.addEventListener('load',function(){probe();setTimeout(probe,300);setTimeout(probe,1200);});}
})();
<\/script>`;
...
function applyCustomBackground(html) {
    ...
    // Инжектируем зонд перед </body> или в конец
...
// Запоминаем последний обработанный маршрут, чтобы не рендерить дважды при hash-переходах
let _lastRenderedPath = '';

function onPopState() {
    const newPath = location.pathname;
    // Если pathname не изменился (только hash) — просто скроллим, не ре-рендерим
    if (newPath === _lastRenderedPath && location.hash) {
...
async function routeAndRender() {
    _lastRenderedPath = location.pathname;
...
function handleClick(e) {
    ...
    // Same path + anchor → просто скроллим, НЕ пишем в историю
