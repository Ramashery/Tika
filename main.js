...
// Применяет/снимает класс light-bg в зависимости от яркости
function applyBgClass(isLight) {
    document.body.classList.toggle('light-bg', isLight);
}

// Слушатель ответов от iframe с цветом фона
window.addEventListener('message', e => {
    if (e.data && e.data.type === 'bg-color') {
        const { r, g, b } = e.data;
...
// Инжектируем в HTML фона скрипт, который замеряет средний цвет и шлёт postMessage
function injectBgProbe(html) {
    const probe = `
<script>
(function() {
    function sendColor() {
        ...
        ctx.drawImage(document.body.firstElementChild || document.documentElement, 0, 0, 16, 16);
        // Fallback: читаем computed background-color body/html
        var el = document.body;
        var bg = getComputedStyle(el).backgroundColor;
        var m = bg.match(/\\d+/g);
        if (m && m.length >= 3) {
            window.parent.postMessage({type:'bg-color',r:+m[0],g:+m[1],b:+m[2]}, '*');
            return;
        }
        // Если не удалось — считаем тёмным
        window.parent.postMessage({type:'bg-color',r:0,g:0,b:0}, '*');
    }
    if (document.readyState === 'complete') { setTimeout(sendColor, 100); }
    else { window.addEventListener('load', function(){ setTimeout(sendColor, 100); }); }
})();
<\/script>`;
...
function applyCustomBackground(html) {
    ...
    const instrumentedHtml = injectBgProbe(html);
    const blob = new Blob([instrumentedHtml], { type: 'text/html' });
...
async function initApp() {
    ...
    // Фиксируем начальный URL как полноценный history-entry,
    // чтобы первое нажатие «назад» корректно срабатывало через popstate.
