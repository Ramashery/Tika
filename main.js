...
// --- CUSTOM BACKGROUND ---
// Скрипт, инжектируемый в каждый фоновый iframe.
// Через 300ms и 900ms после загрузки сэмплирует пиксели canvas (если есть)
// или читает backgroundColor, и отправляет яркость (0-1) через postMessage.
const BG_LUMINANCE_SCRIPT = `<script>
(function() {
    function lum(r,g,b){
        var c=[r/255,g/255,b/255].map(function(v){return v<=0.04045?v/12.92:Math.pow((v+0.055)/1.055,2.4);});
        return 0.2126*c[0]+0.7152*c[1]+0.0722*c[2];
    }
    function sample(){
        try {
            var L=0, found=false;
            // 1. Canvas-элемент в документе
            var cvs=document.querySelector('canvas');
            if(cvs){try{
                var cx=cvs.getContext('2d');
                if(cx){
                    var w=cvs.width,h=cvs.height;
                    var pts=[[w/2,h/2],[w*.25,h*.25],[w*.75,h*.25],[w*.25,h*.75],[w*.75,h*.75]];
                    var sum=0,n=0;
                    pts.forEach(function(p){try{
                        var px=cx.getImageData(Math.floor(p[0]),Math.floor(p[1]),1,1).data;
                        if(px[3]>10){sum+=lum(px[0],px[1],px[2]);n++;}
                    }catch(e){}});
                    if(n>0){L=sum/n;found=true;}
                }
            }catch(e){}}
            // 2. background-color body
            if(!found){
                var bg=getComputedStyle(document.body).backgroundColor;
                var m=bg.match(/rgba?\\((\\d+),\\s*(\\d+),\\s*(\\d+)/);
                if(m&&!(+m[1]===0&&+m[2]===0&&+m[3]===0)){L=lum(+m[1],+m[2],+m[3]);found=true;}
            }
            // 3. background-color html
            if(!found){
                var bg2=getComputedStyle(document.documentElement).backgroundColor;
                var m2=bg2.match(/rgba?\\((\\d+),\\s*(\\d+),\\s*(\\d+)/);
                if(m2&&!(+m2[1]===0&&+m2[2]===0&&+m2[3]===0)){L=lum(+m2[1],+m2[2],+m2[3]);}
            }
            window.parent.postMessage({type:'bg-luminance',value:L},'*');
        } catch(e){
            window.parent.postMessage({type:'bg-luminance',value:0},'*');
        }
    }
    window.addEventListener('load',function(){setTimeout(sample,300);setTimeout(sample,900);});
})();
<\/script>`;

let _bgMessageHandler = null;
...
function applyCustomBackground(html) {
    ...
    if (!html || !html.trim()) {
        if (iframe) {
            ...
            setTimeout(() => { if (iframe) iframe.srcdoc = ''; }, 800);
        }
        document.body.classList.remove('light-bg');
        if (_bgMessageHandler) { window.removeEventListener('message', _bgMessageHandler); _bgMessageHandler = null; }
        return;
    }
    if (!iframe) {
    ...
    if (_bgMessageHandler) window.removeEventListener('message', _bgMessageHandler);
    _bgMessageHandler = (e) => {
        if (!e.data || e.data.type !== 'bg-luminance') return;
        document.body.classList.toggle('light-bg', (e.data.value || 0) > 0.35);
    };
    window.addEventListener('message', _bgMessageHandler);

    // Инжектируем скрипт замера яркости в srcdoc
    const injected = html.includes('</body>')
        ? html.replace('</body>', BG_LUMINANCE_SCRIPT + '</body>')
        : html + BG_LUMINANCE_SCRIPT;

    // srcdoc не создаёт запись в history браузера
