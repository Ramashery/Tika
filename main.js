...
// --- CUSTOM BACKGROUND ---
// FIX: используем srcdoc вместо blob URL — blob URL добавляет запись в history,
// из-за чего кнопка «назад» срабатывала только со второго нажатия.
function applyCustomBackground(html) {
    ...
    iframe.onload = () => {
        ...
        // FIX 2: определяем яркость фона и переключаем класс light-bg
        try {
            const iDoc = iframe.contentDocument || iframe.contentWindow?.document;
            if (iDoc) {
                const bgEl = iDoc.body || iDoc.documentElement;
                ...
                if (m) {
                    ...
                    document.body.classList.toggle('light-bg', L > 0.35);
                } else {
                    document.body.classList.remove('light-bg');
                }
            }
    ...
    // srcdoc не создаёт запись в history браузера — в отличие от src с blob URL
    iframe.srcdoc = html;
}
