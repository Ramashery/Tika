...
function normalizeUrl() {
    ...
    // 1. Collapse multiple consecutive slashes
    path = path.replace(/\/{2,}/g, '/');

    // 2. Lowercase only segments AFTER /Tika — base must stay capitalized (GitHub Pages is case-sensitive)
    const baseLower = BASE_PATH.toLowerCase(); // '/tika'
    if (path.toLowerCase().startsWith(baseLower)) {
        const rest = path.slice(BASE_PATH.length); // everything after '/Tika'
        path = BASE_PATH + rest.toLowerCase();
    } else {
        // Doesn't start with /Tika at all — lowercase everything, reconstruct base
        path = path.toLowerCase().replace(/^\/tika/, BASE_PATH);
    }

    // 3. Ensure trailing slash
    if (!path.endsWith('/')) path += '/';

    const normalized = path + window.location.search + window.location.hash;
